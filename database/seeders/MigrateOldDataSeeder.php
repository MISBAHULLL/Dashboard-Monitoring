...
class MigrateOldDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Migrasi Team
        $oldTeams = DB::connection('old_mtpo')->table('team')->get()->map(fn ($row) => (array) $row);
        $teamMap = [];
        $teamCount = 0;
        ...
        // Migrasi Client
        $oldClients = DB::connection('old_mtpo')->table('client')->get()->map(fn ($row) => (array) $row);
        $clientMap = [];
        $clientCount = 0;
        ...
        $oldTasks = DB::connection('old_mtpo')->table('task')->get()->map(fn ($row) => (array) $row);
        $taskCount = 0;
        $skippedCount = 0;
        ...
        // Migrasi Documents
        $docCount = 0;
        $hasDokumen = Schema::connection('old_mtpo')->hasTable('dokumen');

        if ($hasDokumen) {
            $oldDokumens = DB::connection('old_mtpo')->table('dokumen')->get()->map(fn ($row) => (array) $row);
            ...
            foreach ($oldDokumens as $oldDoc) {
                ...
                $oldTaskWithDocArr = (array) $oldTaskWithDoc;
                $clientId = $clientMap[$oldTaskWithDocArr['faskes']] ?? null;
                if (! $clientId) {
                    continue;
                }
                ...
                foreach ($oldTasksWithDoc as $linkedTask) {
                    ...
                    $newTask = Task::query()->where('title', $linkedTask['fitur'])
                        ->where('client_id', $linkedClientId)
                        ->first();

                    if ($newTask) {
                        DB::table('document_task')->insertOrIgnore([
                            'document_id' => $docId,
                            'task_id' => $newTask->id,
                            'created_at' => now(),
                            'updated_at' => now(),
        ...
        $logCount = 0;
        $hasLog = Schema::connection('old_mtpo')->hasTable('log_tgl_release');

        if ($hasLog) {
            $oldLogs = DB::connection('old_mtpo')->table('log_tgl_release')->get()->map(fn ($row) => (array) $row);
            ...
            foreach ($oldLogs as $oldLog) {
                ...
                $oldTaskArr = (array) $oldTask;
                $clientId = $clientMap[$oldTaskArr['faskes']] ?? null;
                $newTask = Task::query()->where('title', $oldTaskArr['fitur'])
